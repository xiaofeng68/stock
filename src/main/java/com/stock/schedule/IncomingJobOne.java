...
import com.stock.runnable.IncomingRunnable;
 ...
 * 
 */
public class IncomingJobOne extends StatefulMethodInvokingJob {
	private Logger log = LoggerFactory.getLogger(IncomingJobOne.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		ThreadPool threadPool = new ThreadPool(20, "IncomingTheme"); // 创建一个有个3工作线程的线程池
		try {
			Thread.sleep(500);
		} catch (InterruptedException e3) {
			e3.printStackTrace();
		} // 休眠500毫秒,以便让线程池中的工作线程全部运行
		int year = stockWarnTrackService.getLastYear();
		// 运行任务
		for (String code : codeList) {
			if (StockUtils.isStock(code))
				threadPool.execute(new IncomingRunnable(stockWarnTrackService,
						code, year));
		}
