...
import com.stock.runnable.HolderRunnable;
...
/**
 * 个股十大流通股东Job 同步任务执行：每次执行都是new了一个新的执行类，具有线程安全性
 * 
 * @author yu_qhai
 * 
 */
public class StockHolderJobOne extends StatefulMethodInvokingJob {
	private Logger log = LoggerFactory.getLogger(StockHolderJobOne.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		ThreadPool threadPool = new ThreadPool(20, "HolderTheme"); // 创建一个有个3工作线程的线程池
		try {
			Thread.sleep(500);
		...
		criteria.put("state", "出逃");
		stockWarnTrackService.updateHolderState(criteria);
		// 运行任务
		for (String code : codeList) { // 创建6个任务
			if (StockUtils.isStock(code)){// 是个股才去更新
				threadPool.execute(new HolderRunnable(stockWarnTrackService,
						code));
			}
		}
