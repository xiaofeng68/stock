...
public class ThreadPool extends ThreadGroup {  
    ...
    public ThreadPool(int poolSize,String threadPoolID) {  //poolSize 表示线程池中的工作线程的数量  
    ...
    public synchronized void execute(Runnable task) {  
        ...
        if(task != null) {  
            ...
            notify();           //唤醒r个正在getTask()方法中待任务的工作线程  
        }  
    }  
...
//        log.debug("工作线程"+threadid+"开始执行任务...");  
	...
	public static void main(String[] args) throws InterruptedException {  
        ThreadPool threadPool = new ThreadPool(10,"1"); //创建一个有个3工作线程的线程池  
        Thread.sleep(500); //休眠500毫秒,以便让线程池中的工作线程全部运行  
        //运行任务  
        for (int i = 0; i <=2664 ; i++) { //创建6个任务  
